...
private:
    ...
    struct node {
        ...
        node(T val, std::size_t /*level*/, node* nil_ptr) : data(val) {
    ...
    struct ThreadSlab {
        ...
        void recycle(node* n, node* nil_ptr) {
            n->~node();
            // Reutilizamos forward[0] como enlace de la free-list
            // (el nodo está fuera de la skiplist, nadie más lo toca)
